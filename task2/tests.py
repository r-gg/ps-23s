...
class MyTestCase(unittest.TestCase):
    ...
    def test_simple_configs(self):
        ...
        self.assertEqual(  
            '3',
            get_output(
                '(x->y->plus (mult x x) y) 1 2'))  

        self.assertEqual(  
            '6',
            get_output(
                '(x->y->plus (mult x x) y) 2 2')
        )  
    ...
    def test_list_example(self):
        ...
        # Prod instead of sum
        self.assertEqual(
            '{list = (c->(f->(x->cond (c x) {val = x, nxt = list c f (f x)} {}))), reduce = (f->(x->(lst->cond lst (f (reduce f x (lst nxt)) (lst val)) x))), range = (a->(b->list ((x->minus b x)) ((x->plus 1 x)) a)), prod = (lst->reduce ((x->(y->mult x y))) 1 lst)} 24',
            get_output(
                '{list = c -> f -> x -> cond (c x) { val = x, nxt = list c f (f x) } {}, reduce = f -> x -> lst -> cond lst (f (reduce f x (lst nxt)) (lst val)) x, range = a -> b -> list (x -> minus b x) (x -> plus 1 x) a, prod = lst -> reduce (x -> y -> mult x y) 1 lst} prod (range 1 5)')
        )
    ...
    def test_contexts_and_defining_environments_through_records(self):
        self.assertEqual(
            '{} 3',
            get_output('{} {a = 1} (x -> plus a x) 2')
        )
    ...
    def test_lists(self):
        self.assertEqual(
            ...
            get_output(
                '{list = c -> f -> x -> cond (c x) { val = x, nxt = list c f (f x) } {}, range = a -> b -> list (x -> minus b x) (x -> plus 1 x) a} range 1 2')
        )
    ...
    def test_create_list_with_range(self):
        self.assertEqual(
            ...
            get_output(
                '{list = c -> f -> x -> cond (c x) { val = x, nxt = list c f (f x) } {}, range = a -> b -> list (x -> minus b x) (x -> plus 1 x) a} range 1 7')
        )
